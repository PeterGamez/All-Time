...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (gift.status.code == 'SUCCESS') {

            let premium = JSON.parse(fs.readFileSync(`../db/premium.json`, "utf8"))

            if (!premium.user[message.author.id]) premium.user[message.author.id] = { "credit": 0 }

            premium.user[message.author.id].credit = Number(premium.user[message.author.id].credit + gift.data.my_ticket.amount_baht)
        ...
        function type  () {
            ...
            if (stats == 'SUCCESS') return message.channel.send(`ขอขอบคุณเงินสนับสนุนจาก ${gift.data.owner_profile.full_name}\nเป็นจำนวน ${gift.data.my_ticket.amount_baht} บาท`)
            return;
        }
        return type()
    }
