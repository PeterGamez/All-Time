...
module.exports = {
    ...
    run: async (client) => {
        ...
        Promise.all(results).then(async shard => {
            ...
            await client.channels.cache.get('id').setName(`『💻』 System : ${client.config.version}`)
            await client.channels.cache.get('id').setName(`『📖』 Servers : ${formatter.format(shard[3])}`)
            await client.channels.cache.get('id').setName(`『👤』 Users : ${formatter.format(shard[4])}`)
            await client.channels.cache.get('id').setName(`『💬』 Channels : ${formatter.format(shard[5])}`)
        })
